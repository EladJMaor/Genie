...
Quiz.prototype.checkAnswers = function(flagUnanswered) {
	...
	if (unansweredQs.length === 0 || !flagUnanswered) {
		...
		for (i=0; i < unansweredQs.length; i++) {
            ...
            var title_container = unansweredQs[i].getElementsByClassName(this.Classes.QUESTION_TITLE)[0];
            var title_text = title_container.children[0];
            console.log("myTitle "+title_container," " , "my title text "+title_text , " ", "my warning "+warning);
            title_container.insertBefore(warning,title_text);
